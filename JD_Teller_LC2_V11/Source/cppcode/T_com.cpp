...
_BOOL get_lasal_id(_LASALID *p, _ASCII *label, _LSEVARINFO *pi)
{
  ...
  if(cp != NULL)
  {
    ...
    if((pi->POWPRUIC & VARBIT_VARGLOBAL) != VARBIT_VARGLOBAL)
   #endif
    {
   ...
   #if defined(UC_GLOBAL_VAR) || defined(UC_GLOBAL_VAR_REFLIST)
    else if(pi->POWPRUIC & VARBIT_VARGLOBAL)
    {
      ...
      if(GetVarInfo(&p, i) == _TRUE)
      {
        ...
        if(login_PLC(p->station) != NULL)
        {
          if(GetLabel(label, &varobj, i) == _TRUE)
          {
            ...
            if(p->POWPRUIC & VARBIT_VARGLOBAL)
            {
              if(GetRootParent(&tmpvi, label, p->station) == _TRUE)
              {
               #ifdef UC_LINKVL
                _UBYTE no;

                if((tmpvi->memlinkvl.no > 0) && (tmpvi->memlinkvl.ptr != NULL))
                {
                  no = tmpvi->memlinkvl.no - 1;
                  if((tmpvi->memlinkvl.ptr[no].info == VL_STRUCTUREOFFSET) && (tmpvi->memlinkvl.ptr[no].link.no == 1) && (tmpvi->memlinkvl.ptr[no].link.info[0].state == CONST_VAL))
                  {
                    if(tmpvi->lasalid.id != DEFLASALID)
                    {
                      ...
                      if((p->memlinkvl.no > 0) && (p->memlinkvl.ptr != NULL))
                      {
                        ...
                        if((p->memlinkvl.ptr[no].info == VL_STRUCTUREOFFSET) && (p->memlinkvl.ptr[no].link.no == 1) && (p->memlinkvl.ptr[no].link.info[0].state == CONST_VAL))
                        {
                          id.id = tmpvi->lasalid.id + p->memlinkvl.ptr[no].link.info[0].value;
                        }
                      }
                    }
                  }
...
_UWORD get_extendeddataof(_UBYTE *dst, _UDWORD length, _UWORD offset, _UDWORD lasalid, _COMDEF *cd)
{
  _UWORD retcode = 0;
  _UBYTE cmd[20];
  _UBYTE *result;
  ...
  if(TxCommandIntern(I_CMD, 12, cmd, cd, (void**)&result, DEFSVRTYP) == DONE) // length 10 !!
  {
    ...
    if(tmplen > 2)
    {
      ...
      if(tmplen > 250)
      {
        ...
        retcode = 250 + get_extendeddataof(dst+250, tmplen-250, offset+250, lasalid, cd);
      }
      else
      {
    ...
    *(_UWORD*)&cmd[7]  = size + 2; //VarList.GetVariableSize(p->vartype, p->format);
    ...
    if(TxCommandIntern(I_CMD, 9, cmd, &cp->comdef, (void**)&result, DEFSVRTYP) == DONE) // length 10 !!
    {
      ...
      if(length > 250)
      {
        ...
        length = 250 + get_extendeddataof(&tmp[250], length-250, 250, pid->id, &cp->comdef);
        result = tmp;
      }
...
//        *(_UWORD*)&p->refdat.data[offset] = retcode;
       ...
       #ifdef UC_GLOBAL_VAR_REFLIST
        if(plvi->POWPRUIC & VARBIT_VARGLOBAL)
        {
          reg.xtime |= 0x4000;
        }
        else
        ...
        if(permanent == _TRUE)
        {
          StartStopRefresh(cd, p->indexdat.permanent, 0);
        }
        else
        {
          StartStopRefresh(cd, p->indexdat.no_sent-p->indexdat.permanent, 1);
        }
      }
    }
  }
...
//#ifdef UC_GLOBAL_VAR
#if defined(UC_GLOBAL_VAR) || defined(UC_GLOBAL_VAR_REFLIST)
_BOOL get_global(void *ret, _VARINFO *pvi)
{
  _BOOL retcode = _FALSE;
  _UBYTE tmp[50];
  _UBYTE size;
...
//           #ifdef UC_GLOBAL_VAR
            ...
            if(pvi->POWPRUIC & VARBIT_VARGLOBAL)
            {
              if(get_global(directdata, pvi) == _TRUE)
                state = _TRUE;
            }
            else
           #endif
