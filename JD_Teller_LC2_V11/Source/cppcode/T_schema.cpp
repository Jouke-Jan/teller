...
_BOOL _Scheme::CallSub(_UDWORD *dst, _MEM_SINGLESCHEME *ps, _VARLABEL *pl)
{
  _RESULT       result;
  _VARIABLE     variable;
  _BOOL         st;
...
_BOOL _Scheme::Call(_UDWORD *dst, _MYSCHEME *pd, _VARLABEL *link, _UDWORD def)
{
  *dst = def;
  ...
  if(pd->schemeno < MemScheme.no)
  {
    ...
    if(chk_VARLABEL_Makro(link)) // Makro, speed: nicht auf true oder false abfragen 
      return CallSub(dst, &ps->memsinglescheme, link);
    return _FALSE;
  }
  else if(pd->overload.memsinglescheme.no > 0)
  {
    ...
    //if(chk_VARLABEL(link) == _TRUE)
    if(chk_VARLABEL_Makro(link)) // Makro, speed: nicht auf true oder false abfragen 
      return CallSub(dst, &pd->overload.memsinglescheme, link);
    return _FALSE;
  }
...
_BOOL _Scheme::Call(_UDWORD *dst, _LINKSCHEME *pd, _UDWORD def)
{
  if((pd->myscheme.schemeno < MemScheme.no)||(pd->myscheme.overload.memsinglescheme.no > 0)) // speed
    return(Call(dst, &pd->myscheme, &pd->varlabel, def));
