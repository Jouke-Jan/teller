...
_BOOL _VarList::VarDummyGet(_RESULT *res, _VARLABEL *vl)
{
  if(chk_VARLABEL_Makro(vl))
  {
    ...
    {
      if(vl->info[0].value >= VARNO_INSERTFLAG)
      {
        switch(vl->info[0].value)
        {
          case VARNO_ALTGRFLAG:
            res->ftype = RES_NUMERIC;
            res->value = VarDummyData[2];
            res->rmin  = 0;
            res->rmax  = 1;
            return _TRUE;
...
_BOOL _VarList::VarDummySet(_VARLABEL *vl, _RESULT *res)
{
  if(chk_VARLABEL_Makro(vl))
  {
    if((vl->info[0].state = VAR_VAL) && (vl->info[0].value >= VARNO_INSERTFLAG))
    {
...
_BOOL _VarList::InsertUnitInfo(_VARINFO *p)
{
  ...
  if((p->vartype == BINARY) || (p->vartype == FCONSTANT))
  {
    ...
    if(pu != NULL)
    {
      ...
      if(GetSystemData(&res, &var, &pu->posdezpt, _TRUE, _TRUE) == _TRUE)
      {
        switch(res.ftype)
        {
          case RES_REAL     : p->posdezpt = (_UBYTE)FTOL(get_pREAL((_REAL*)&res.value) + 0.5); // dr1850, sa23917
                              break;
          case RES_NUMERIC  :
          case RES_CONSTANT : p->posdezpt = (_UBYTE)res.value;
                              break;
          default           : break;       
        }

        if(GetSystemData(&res, &var, &pu->xdigits, _TRUE, _FALSE) == _TRUE)
        {
...
_BOOL _VarList::GetMinMax(_RESULT *res, _VARIABLE *p, _VARLABEL *pvl, _BOOL dolimit)
{
  _RESULT   mm;
  _VARIABLE tmp;
  _VARLABEL *vl;
  ...
  if(dolimit == _TRUE)
  {
    if(pvl->info[0].state != PARA_VAL)  
    {
      ...
      if(vl->no > 0)
      {
        ...
        {
          if(GetSystemData(&mm, &tmp, vl, _TRUE, _FALSE) == _TRUE)
          {
...
_BOOL _VarList::GetValueIntern(_RESULT *res, _VARIABLE *p, _VARLABEL *vl, _BOOL wait4plc, _BOOL dolimit)
{
  _BOOL       retcode = _FALSE;
  _UWORD      index;
  _COMPORT    *cd;
  ...
  if(GetVariable(p, vl) == _TRUE)
  {
    if(GetStateScheme(p->no, NOCARE_DEFAULT) != STATE_INVISIBLE)
    {
      ...
      if(p->info.vartype == BIT)
      {
        ...
        _IVALUE bval;
        if(GetValueByNo(Data.MemVarSymbol.ptr[p->no].scope,  &bval, _TRUE) == _TRUE)
        {
          if(format_BIT(res, &p->info, &bval) == _TRUE)
          {
            return(GetMinMax(res, p, vl, dolimit));
          }
        }
      }
     #endif
...
//        case BITMASK   : retcode = GetBitmask(res, &p->info, vl);  break;
      ...
      if(retcode == _TRUE)
      {
        retcode = GetMinMax(res, p, vl, dolimit);
      }
...
_BOOL _VarList::GetSystemData(_RESULT *res, _VARIABLE *p, _VARLABEL *vl, _BOOL wait4plc, _BOOL dolimit)
{
  _BOOL     retcode = _TRUE;
  _VARLABEL tmpvl;
  ...
  switch(vl->info[0].state)
  {
    case PARA_VAL  : retcode = _FALSE;
                     ...
                     if(vl->no > 1)
                       retcode = GetValueIntern(res, p, vl, wait4plc, dolimit);
                     break;
    case CRC_VAL   : tmpvl = *vl;
                     tmpvl.info[0].value = GetVarNoByCRC(tmpvl.info[0].value);
                     tmpvl.info[0].state = VAR_VAL;
                     retcode = GetValueIntern(res, p, &tmpvl, wait4plc, dolimit);
                     break;
    case VAR_VAL   : retcode = GetValueIntern(res, p, vl, wait4plc, dolimit);
                     break;
    case BOOL_VAL  :
...
_BOOL _VarList::GetGlobalValue(_RESULT *res, _VARIABLE *p, _VARLABEL *vl, _BOOL dolimit)
{
  ...
  if(get_global(&tmp, &p->info) == _TRUE)
  {
...
_BOOL _VarList::GetSystemData(_RESULT *res, _VARIABLE *p, _VARLABEL *vl, _BOOL wait4plc, _BOOL dolimit)
{
  ...
  switch(vl->info[0].state)
  {
    case PARA_VAL  : // letzter _VARSINGLE beinhaltet POINTER_VAL und pointer auf daten (wert)
                     if(vl->no > 1)
                       retcode = GetValueIntern(res, p, vl, wait4plc, dolimit);
                     break;
    case VAR_VAL   : retcode = GetValueIntern(res, p, vl, wait4plc, dolimit);
                     break;
    case BOOL_VAL  :
    case PARA_CONST: init_VARIABLE(p);
                     if(vl->no > 1)
                     {
                       if((vl->info[1].state == POINTER_VAL)&&(vl->info[1].value != 0))
                       {
                     ...
                     break;
    case CONST_VAL : init_VARIABLE(p);
                     res->ftype = RES_CONSTANT;
                     res->value = vl->info[0].value;
                     res->ptr   = &res->value;
                     res->rmin  = res->value;
    ...
    case REAL_VAL:   init_VARIABLE(p);
                     ...
                     retcode    = _TRUE;
                     break;
    case INDEX_VAL : init_VARIABLE(p);
                     res->ftype = RES_CONSTANT;
                     res->value = GetIndexValue(vl);
                     res->ptr   = &res->value;
                     res->rmin  = res->value;
...
//   #ifdef UC_GLOBAL_VAR
    ...
    if(p->POWPRUIC & VARBIT_VARGLOBAL)
    {
      ...
      *(_UDWORD*)&tmpx[0] = p->lasalid.id + (p->bitno & 0x1F);

      tmpx[4] = GetVariableSize(p->vartype, p->format);
      *(_UDWORD*)&tmpx[5] = *(_UDWORD*)ptr;//GetVariableSize(p->vartype, p->format);
      if(TxCommandIntern(I_SET_DATA, 9, tmpx, &cp->comdef, (void**)&result, DEFSVRTYP) == DONE)
      {
        retcode = _TRUE;
      }
...
_BOOL _VarList::GetValue(_VARLABEL *pvl, _IVALUE *dest, _BOOL wait4plc)
{
  _RESULT   res;
  _VARIABLE var;

  if(GetSystemData(&res, &var, pvl, wait4plc, _FALSE) == _TRUE)
  {
    switch(res.ftype)
    {
      case RES_CONSTANT :
...
_LSEVARINFO *_VarList::GetRootInfo(_UDWORD varno)
{
  ...
  if(varno < Data.no)
  {
    scope = Data.MemVarSymbol.ptr[varno].scope;
    if(scope >= Data.no)
