...
//                           OS_MoveCursor(osevent.eventX, osevent.eventY);
  ...
  // tastatur-berichtigung für km-terminal mit rtk32 (otto)
  ...
  if(Event.ftype != _EVENT_NONE)
  {
    ...
    if((Event.ftype == _EVENT_KEYPRESS)||(Event.ftype == _EVENT_KEYRELEASE))
    {
      if(Event.scancode == 0xFFFF)
        init_EVENT_Makro(&Event); // wird verworfen, weil die der defaultwert für den scancode ist (defaulthotkeycode bei button)
    }
