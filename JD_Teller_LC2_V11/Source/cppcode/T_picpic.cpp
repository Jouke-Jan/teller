...
_BOOL _Picture::AutomaticSeekInput(_BOOL just_welcome_austria)
{
  ...
  if((Pic.FirstInput.x >= room.xy1.x)&&(Pic.FirstInput.x <= room.xy2.x)&&(Pic.FirstInput.y >= room.xy1.y)&&(Pic.FirstInput.y <= room.xy2.y))
  {
    retcode = SeekHid(&Pic.FirstInput, _TRUE, _FALSE, _SELECTOR_DEFAULT, NULL);
    if(just_welcome_austria == _TRUE)
      return(retcode);
  }
...
void _Picture::Refresh(_DOT *offset)
{
 #ifdef _CHANGELOOKUP
  if(GlobalChangeSys)
  {
...
void _Picture::RunIntern(_Window *PWin, _EVENT *pe, _DOT *po)
{
  _UWORD      i;
 #ifdef HID
  _DOT        offset;
  ...
  else
  {
    ...
    else if((pe->ftype == _EVENT_HIDPRESS)||(pe->ftype == _EVENT_HIDRELEASE)||(pe->ftype == _EVENT_HIDMOVE)||(pe->ftype == _EVENT_TRIGGER)||(pe->ftype == _EVENT_PREHIDPRESS))
    {
      ...
      for(i=0; i<Pic.No; i++)
      {
        ...
        {
          found_something_by_hid(FOUND_BUTTON);
          state = _TRUE;
          break;
        }
      }
      ...
      if((pe->ftype == _EVENT_HIDPRESS)&&(state == _FALSE))
      {
        ...
        {
          found_something_by_hid(FOUND_INPUT);
          state = _TRUE;
        }
      }
      ...
      if(((pe->ftype == _EVENT_HIDPRESS)||(pe->ftype == _EVENT_HIDRELEASE))&&(state == _FALSE))
      {
        if(Theme.IsAction == _TRUE) // test ob überhaupt ein thema mit actionbutton vorhanden ist
        {
          ...
          {
            found_something_by_hid(FOUND_CANCLE);
...
_BOOL _Picture::Overload(_VARLABEL *ov)
{
  _UWORD i;
