...
_UWORD tayu_keytab[] = {'1', 'S', 'T', 'U', '1', 0, 0, 0, 0, 0,
                        '2', 'V', 'W', 'X', '2', 0, 0, 0, 0, 0,
                        '3', 'Y', 'Z', '3',   0, 0, 0, 0, 0, 0,
                        '4', 'J', 'K', 'L', '4', 0, 0, 0, 0, 0,
                        '5', 'M', 'N', 'O', '5', 0, 0, 0, 0, 0,
...
{
 #ifdef USER_TAYU
  set_MULTIKEYBOARD(11, tayu_keytab, 500);
 #endif
...
void _Picture::ShowChangedEditor(_EDITOR *ped)
{
  ...
  switch(ped->input.io.typ)
  {
    ...
    case AS_THEURGE      :
      ...
      {
        for(i=0; ped->chredit.fstring[i]!=0; i++)
          stars[i] = '*';
...
_BOOL _Picture::EditAscii(_EVENT *pe)
{
  ...
  if((pe->scancode == _RIGHT)||(pe->scancode == _LEFT)||(pe->scancode == _POS1)||(pe->scancode == _END))
  {
    if(Pic.Editor.chredit.change == _FALSE)
    {
      Pic.Editor.chredit.no = StrLen(Pic.Editor.chredit.fstring, sizeof(_CHAR));
...
*/
                 ...
                 #endif
                  {
//                    retcode = run_CHREDIT(&Pic.Editor.chredit, pe->scancode, EditorInsertMde, _FALSE, pe->is_character);
