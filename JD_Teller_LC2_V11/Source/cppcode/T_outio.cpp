...
_BOOL _SubPicture::SingleChangeIo(_IO *pio, _RESULT *pr, _VARIABLE *pv, _Picture *Ppic, _BOOL input)
{
  ...
  {
    if(ColorScheme.Call(&backhi, &pio->dyncolback, GetBackColor_Makro(pio->colback)) == _TRUE)
    {
      if(ColorScheme.Call(&backlo, &pio->dyncolfront, GetPenColor_Makro(pio->colback)) == _TRUE)
      {
        if(ColorScheme.Call(&framehi, &pio->dyncoltl, GetBackColor_Makro(pio->colframe)) == _TRUE)
        {
          if(ColorScheme.Call(&framelo, &pio->dyncolbr, GetPenColor_Makro(pio->colframe)) == _TRUE)
          {
            if(ColorScheme.Call(&txt, &pio->dyncoltxt, GetPenColor_Makro(pio->coltxt)) == _TRUE)
            {
