...
_TOOLERROR scan_line(_LSEVARINFO **ppvi, _VARLABEL *vl, _RESULT *result, _ASCII *line, _STATISTIC *filter,  _UDWORD version)
{
  _ASCII      chr;
  _UDWORD     varno;
  _UWORD      counter;
  ...
  chr     = ',';
  counter = (version < 100)? 0 : 0;  // wegen warning: parameter never used
  run     = 1;
  tmp     = get_temptext();
    ...
    if(chr == ',')
    {
      switch(counter)
      {
        case 0 : // label
          ...
          varno = VarList.GetVarNo((_ASCII*)tmp, get_station(line));
        ...
        case 1 : // data
          ...
          if(VarList.GetVarInfo(&vi, varno) == _TRUE)
          {
            ...
            if((IsFilter(&vi->statistic, filter)) == _TRUE)
            {
              vl->no = 1;
...
_BOOL _ToolCatalogue::SingleLoad(_ASCII *line)
{
  ...
  toolerror = scan_line(NULL, &vl, &result, line, &ToolData.Filter, ToolData.Version);
