...
{
  return ((p->x >= pr->xy1.x) && (p->x <= pr->xy2.x) && (p->y >= pr->xy1.y) && (p->y <= pr->xy2.x))? _TRUE : _FALSE;
}
...
void draw_SCROLLBAR(_SCROLLBAR *p)
{
  ...
  if(p->valid == _TRUE)
  {
    ...
    if(p->horizontal == _TRUE)
    {
      ...
      level_triangle(&room, _LEFT, p->col_text);

      room       = p->room;
      room.xy1.x = p->room.xy2.x - B_WIDTH + 1;
      draw_scrbutton(&room, p->col_front, p->col_frame, p->frametype, _FALSE);
      level_triangle(&room, _RIGHT, p->col_text);
    ...
    else
    {
      ...
      draw_scrbutton(&room, p->col_front, p->col_frame, p->frametype, _FALSE);
      level_triangle(&room, _UP, p->col_text);
      
      room       = p->room;
      room.xy1.y = room.xy2.y - B_HEIGHT + 1;
      draw_scrbutton(&room, p->col_front, p->col_frame, p->frametype, _FALSE);
      level_triangle(&room, _DOWN, p->col_text);
      ...
      room.xy2.y = room.xy1.y + B_HEIGHT - 1;;
      draw_scrbutton(&room, p->col_front, p->col_frame, p->frametype, p->active);
//      draw_scrbutton(p->room.xy1.x, xy, p->room.xy2.x, xy+B_HEIGHT-1, p->col_back, p->col_frame, p->frametype, p->active);
    }
...
void dec_CAROUSEL(_MOTION *p)
{
  ...
  if(idx < p->memkaru.no)
  {
    if((p->infinite == _TRUE) || (idx != (p->memkaru.no - 1)))
    {
      _KARU *pk = &p->memkaru.ptr[idx];
      
      if(p->horizontal == _TRUE)
      {
        //retcode =  1 + (((pk->room.xy2.x - pk->room.xy1.x + 1) * 2) / 3);
        retcode =  1 + ((((pk->room.xy2.x - pk->room.xy1.x + 1) * (100 - p->skip_percent)) + 50) / 100);
      }
      else
      {
        //retcode =  1 + (((pk->room.xy2.y - pk->room.xy1.y + 1) * 2) / 3);
        retcode =  1 + ((((pk->room.xy2.y - pk->room.xy1.y + 1) * (100 - p->skip_percent)) + 50) / 100);
      }
    }
  }
  
  set_pos_CAROUSEL(vl, get_pos_CAROUSEL(vl) - retcode);
  p->swim = _TRUE;
}
...
void inc_CAROUSEL(_MOTION *p)
{
  ...
  if(idx < p->memkaru.no)
  {
    ...
    if(idx < p->memkaru.no)
    {
      ...
      if(p->horizontal == _TRUE)
      {
        //retcode = p->gap + 1 + (((pk->room.xy2.x - pk->room.xy1.x + 1) * 2) / 3);
        retcode = p->gap + 1 + ((((pk->room.xy2.x - pk->room.xy1.x + 1) * p->skip_percent) + 50) / 100);
      }
      else
      {
        //retcode = p->gap + 1 + (((pk->room.xy2.y - pk->room.xy1.y + 1) * 2) / 3);
        retcode = p->gap + 1 + ((((pk->room.xy2.y - pk->room.xy1.y + 1) * p->skip_percent) + 50) / 100);
      }
    }
  }
...
void draw_CAROUSEL(_MOTION *pm, _DOT *poffs, _Picture *Ppic)
{
  ...
  for(i=0; i<p->no; i++)
  {
    ...
    if((old == _FALSE) && (p->ptr[i].gesamt == _TRUE))
    {
...
void swim_CAROUSEL(_MOTION *p)
{
  if((p->memkaru.no > 0) && (timediff(p->swimtime) > 40))
  {
...
_BOOL _SubPicture::Refresh_CAROUSEL(_UWORD actno, _MOTION *p, _DOT *offset, _ZOOM *zoom, _BOOL init, _Picture *Ppic)
{
