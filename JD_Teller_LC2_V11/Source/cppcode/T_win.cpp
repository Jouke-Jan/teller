    ...
    switch(P_Window->RunIntern())
    {
      ...
      case RUN_CLOSE :                 // close window and restore background
        ...
        if((SetUp.GetUseTouchEditor() == _TRUE)&&(te == _TRUE))
        {
          ...
          else if((wima == IMA_TOUCHEDITOR)||(wima == IMA_POPUP))
          {
...
//        AddCmd(CMD_OUTSOFTKEY, Softkey.GetActFloor(), 0, _FALSE);
  ...
  if(IsReady == _TRUE)
  {
   ...
   #ifdef UC_REFRESH_JUST_TOP_WINDOW
    if(P_Window == NULL)
   #endif
    {
      ActivateRegion(Region);
      ...
      if(P_Picture != NULL)
      {
        P_Picture->RunIntern(this, &event, &PicOffset);                 // do picture
        if(P_Window == NULL)
     ...
     #endif
      {
            ...
            (event.dot.y < room.xy1.y) || (event.dot.y > room.xy2.y))
        {
          if(MessBoxIndex == 0xFFFFFFFF) // nur wenn es keine messagebox ist !!
          {
            // hid out of window
            Beep(BEEP_ESC);
            found_something_by_hid(FOUND_ESC);
    ...
    if(nst == _TRUE)
    {
      DashBoard.RunIntern(this, &event, &PicOffset);
    }
   #endif
    ...
    if(pwai->ima == IMA_TOUCHEDITOR)
    {
      if(SetUp.GetUseTouchEditor() == _TRUE)
      {
        ...
        if(ServerToEdit != DEFSCOPE)
        {
          ...
          if(P_Picture->Overload(&vl) == _TRUE)
          {
