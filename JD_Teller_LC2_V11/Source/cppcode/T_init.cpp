...
{
  if(p->seqno != DEFSEQNO)
    return Sequencer.Call(p->seqno, (p->invert == 0)? _FALSE : _TRUE);
  else if(p->overload.size > 0)
    return Sequencer.CallCompiler(&p->overload, (p->invert == 0)? _FALSE : _TRUE);
...
_BOOL chk_XXCHKBIT(_XXCHKBIT xxchkbit)
{
  if((xxchkbit & 0x7FFF) != 0x7FFF)
    return Sequencer.Call(xxchkbit & 0x7FFF, (xxchkbit & 0x8000)? _TRUE : _FALSE);
