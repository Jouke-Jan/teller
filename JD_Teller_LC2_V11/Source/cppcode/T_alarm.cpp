...
//    if(LookForAlarmCrc() == _FALSE)
   #endif
    {
      ...
      {
        ScanBar.Run(bs, i);
        if(config_SINGLE_ALARM(psa, _FALSE) == _FALSE)
...
_UWORD _Alarm::GetIndex(_UWORD no)
{
  _UWORD        i;
  _SINGLE_ALARM *pa;
...
void _TermAlarm::ReConfig(_BOOL st)
{
  ...
  for(i=0; i<no; i++)
  {
    ...
    {
      ptr->anything.no = Alarm.GetIndex(ptr->no);
...
_BOOL _TermAlarm::Insert_SINGLEALARM(_MEM_SINGLEALARM *p, _SINGLEALARM *ptr)
{
  ...
  if(p->locked == _FALSE)
  {
    if(p->File.GetError() == _FALSE)
    {
      ...
      pos = (ptr->seqno % p->ringbuffer.no);
      if(p->File.Seek((pos*512)+512, SEEK_SET) == _TRUE)
...
// bei km kann alarm mehrmals aktiv werden ohne je passiv zu sein (randi)
  ...
  if(Project.MakeConfigDPNE(dpne, HISTORYFILENAME, _FALSE) == _TRUE)
  {
    if((History.ptr != NULL)&&(History.ringbuffer.no > 0))
    {
      ...
      if(XFile.Open(LOAD, xdpne) == _TRUE) // kontrolle ob file bereits vorhanden
      {
        ...
        if((XFile.Read_HEADER(&version, HISTORYFILEID) == _TRUE)&&(version <= HISTORYFILEVERSION))
        {
...
void out_standard_alarmline(_SINGLEALARM *p, _ROOM *pr, _COLOR color, _UWORD filter, _SCROLL *ps)
{
  ...
  if(p->entrytype == 0xFFFF)
  {
    ...
    if(p->anything.no == 0xFFFF)
    {
      ...
      p->anything.no = Alarm.GetIndex(p->no);
    }
  ...
  if(Queue == _TRUE)
  {
    ...
    for(i=0; i<TERMALARMPUFFER; i++)
    {
      if(tmp->acknowledge == 2)
      {
        ...
        res.anything.no   = Alarm.GetIndex(res.no);
       #if HISTORYFILEVERSION >= 1
...
void ProcessAlarmRecords(_UBYTE *data, _UDWORD anzahl, _MEM_SINGLEALARM *ptemp, _MEM_SINGLEALARM *phist)
{
  _SINGLEALARM sa, sa1;
  _UBYTE       tmp;
  ...
  while(anzahl)
  {
    ...
    {
      sa.anything.no = Alarm.GetIndex(sa.no);
    }
   #else
    sa.anything.no = Alarm.GetIndex(sa.no);
   #endif
...
void ProcessAlarmRecords202(_UBYTE *data, _UDWORD anzahl, _MEM_SINGLEALARM *ptemp, _MEM_SINGLEALARM *phist, _UDWORD recordsize)
{
  _SINGLEALARM sa, sa1;
  _UBYTE       tmp;
  _UBYTE       *ph = data;
    ...
    *(_UBYTE*) &tx[23] = P_EOL;
    ...
    if(TxCommandIntern(I_CMD, 24, tx, &pcp->comdef, (void**)&rx, DEFSVRTYP) == DONE)
    {
      ...
      if(data != NULL)
      {
        ...
        ProcessAlarmRecords202(data, tmpanz, ptemp, NULL, pcp->alarmRecordSize);  
        anz -= tmpanz;
        start += tmpanz;
      ...
      *(_UBYTE*) &tx[11] = P_EOL;
      ...
      if(TxCommandIntern(I_CMD, 12, tx, &pcp->comdef, (void**)&rx, DEFSVRTYP) == DONE)
      {
        ...
        if(data != NULL)
        {
          ...
          {
            pcp->oldValue += anzahl;
            ProcessAlarmRecords(data, anzahl, ptemp, phist);
          }
        }
        else
        {
...
{
  return Alarm.Get_SINGLE_ALARM(Alarm.GetIndex(alaconfigno));
}

extern "C" _UDWORD LSE_GetAlarmConfigIndex(_UWORD alaconfigno)
{
  _UDWORD retcode = Alarm.GetIndex(alaconfigno);
  if(retcode == 0xFFFF)
    retcode = 0xFFFFFFFF;
