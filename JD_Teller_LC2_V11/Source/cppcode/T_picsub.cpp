...
/*
void doimage_IO(_IO *pio)
{
  // keine dynamische farbe
  // kein statescheme
  // kein virtueller name
  // kein 'theurge'
  // kein statescheme bei referenzierten server --> überladen geht nicht mehr, km bild 60 !!
}


void doimage_PICTEXT(_PICTEXT *pt)
{
  pt->doimage = _TRUE;
//  if(pt->stscheme.schemeno == DEFAULTSCHEME)
//  {
//    if(pt->stscheme.overload.memsinglescheme.no == 0)
//    {
//      if(is_invisible(pt->colback, pt->attrib) == _FALSE)
//      {
//        if(GetPenColor_Makro(pt->coltxt) != INVISIBLE)
//        {
//          pt->doimage = _FALSE;
//          if(pt->frametype != _DEFFRAME)
//          {
//            if(GetPenColor_Makro(pt->colframe) == INVISIBLE)
//              pt->doimage = _TRUE;
//            if(GetBackColor_Makro(pt->colframe) == INVISIBLE)
//              pt->doimage = _TRUE;
//          }
//        }
//      }
...
void  draw_button_cursor(_ROOM, _COLOR, _ATTRIB);
...
_BOOL _SubPicture::Refresh(_UWORD actno, _DOT *offset, _ZOOM *zoom, _Picture *Ppic)
{
 ...
 #ifdef _CHANGELOOKUP
  _UDWORD st = 0;
  if((Ppic != NULL)&&(Ppic->RefCyc == 0))
  {
    if((GlobalChangeSys)||(Picture.actual.cnt_priority == 0))
    {
      // alle inputs für refresh aufrufen
      st = 1;
    }
  }
  else if(Picture.actual.cnt_priority == 0)
  {
    // immer nur 8 inputs für refresh aufrufen und nicht auf GlobalChangeSys achten
    st = 1;
    no = 8;
 ...
 #endif
  {
    for(i=0; i<no; i++)
    {
      ...
      pi  = &Picture.mask.meminput.ptr[Picture.actual.act_input];
      pio = &pi->io;
      Picture.actual.act_input += 1;
  ...
  // do all outputs ***********************************************************
 #ifdef _CHANGELOOKUP
  if((GlobalChangeSys)||(Picture.actual.cnt_priority == 1))
 #endif
  {
  ...
  if((Ppic != NULL)&&(Ppic->RefCyc == 0))
  {
    if((GlobalChangeSys)||(Picture.actual.cnt_priority == 2))
      st = 1;
  }
  else if(Picture.actual.cnt_priority == 2)
  {
    // nicht auf GlobalChangeSys achten
    st = 1;
  }
  if(st)
 #endif
  ...
  // do all objects ***********************************************************
 #ifdef _CHANGELOOKUP
  if((GlobalChangeSys)||(Picture.actual.cnt_priority == 3))
 #endif
  {
    // look into all placed objects
  ...
  Picture.actual.cnt_priority += 1;
  if(Picture.actual.cnt_priority > 3)
  {
    if((Ppic != NULL)&&(Ppic->RefCyc == 0))
      Picture.actual.cnt_priority = 0;
    else if(Picture.actual.cnt_priority > 6)
      Picture.actual.cnt_priority = 0; // 3 leere Durchlaufe damit Eventqueue nicht überläuft
  }

  pa = &Picture.actual;

  if(pa->act_motion < Picture.mask.memmotion.no)
  {
   #ifdef UC_OLD_MOTIONS
    if(Refresh_MOTION(&Picture.mask, pa->act_motion, offset, zoom, _FALSE) == _TRUE)
   #else
    if(Refresh_MOTION(actno, &Picture.mask.memmotion.ptr[pa->act_motion], offset, zoom, _FALSE, Ppic) == _TRUE)
   #endif
      pa->act_motion ++;
  }
  else
  ...
  if(pa->act_colrect < Picture.mask.colrect.no)
  {
    Out_COL_RECT(&Picture.mask.colrect.ptr[pa->act_colrect], offset, zoom, _FALSE);
    pa->act_colrect ++;
  }
  else if(pa->act_colpoly < Picture.mask.colpoly.no)
  {
    Out_COL_POLY(&Picture.mask.colpoly.ptr[pa->act_colpoly], offset, zoom, _FALSE);
    pa->act_colpoly ++;
  }
  else if(pa->act_colcirc < Picture.mask.colcirc.no)
  {
    Out_COL_CIRC(&Picture.mask.colcirc.ptr[pa->act_colcirc], offset, zoom, _FALSE);
    pa->act_colcirc ++;
  }
  else if(pa->act_colpictxt < Picture.mask.colpictxt.no)
  {
		Out_COL_PICTXT(&Picture.mask.colpictxt.ptr[pa->act_colpictxt], offset, zoom, _FALSE);
    pa->act_colpictxt ++;
  }
  else if(pa->act_pictext < Picture.mask.mempictxt.no)
  {
    Out_PICTEXT(&Picture.mask.mempictxt.ptr[pa->act_pictext], offset, zoom, _FALSE);
    pa->act_pictext ++;
  }
	else
  {
...
_BOOL _SubPicture::OutIoIntern(_IO *pio, _DOT *offset, _ZOOM *zoom, _BOOL input, _BOOL init, _UWORD actno, _Picture *Ppic, _BOOL iseditor)
{
  ...
  if(chk_CHKBIT(&pio->chkbit) == _TRUE)
  {
    ...
    {
      retcode = VarList.GetSystemData(&res, &var, &pio->variable, _FALSE, dolimit);
    }
...
_BOOL _SubPicture::Refresh_MOTION(_UWORD actno, _MOTION *p, _DOT *offset, _ZOOM *zoom, _BOOL init, _Picture *Ppic)
{
  ...
  if(p->memkaru.no > 0)
    return Refresh_CAROUSEL(actno, p, offset, zoom, init, Ppic);
...
_BOOL _SubPicture::Out_COL_RECT(_COL_RECT *p, _DOT *offset, _ZOOM *zoom, _BOOL init)
{
  ...
  if(ColorScheme.Call(&colhi, &p->dyncolback, GetBackColor_Makro(p->rectangle.color)) == _TRUE)
  {
    if(ColorScheme.Call(&collo, &p->dyncolfront, GetPenColor_Makro(p->rectangle.color)) == _TRUE)
    {
      if(ColorScheme.Call(&framehi, &p->dyncoltl, GetBackColor_Makro(p->rectangle.colorframe)) == _TRUE)
      {
        if(ColorScheme.Call(&framelo, &p->dyncolbr, GetPenColor_Makro(p->rectangle.colorframe)) == _TRUE)
        {
...
_BOOL _SubPicture::Out_COL_CIRC(_COL_CIRC *p, _DOT *offset, _ZOOM *zoom, _BOOL init)
{
  ...
  if(ColorScheme.Call(&colhi, &p->dyncolback, GetBackColor_Makro(p->circle.color)) == _TRUE)
  {
    if(ColorScheme.Call(&collo, &p->dyncolfront, GetPenColor_Makro(p->circle.color)) == _TRUE)
    {
      if(ColorScheme.Call(&frame, &p->dyncolframe, GetPenColor_Makro(p->circle.colorframe)) == _TRUE)
      {
        circle            = p->circle;
...
_BOOL _SubPicture::Out_COL_PICTXT(_COL_PICTXT *p, _DOT *offset, _ZOOM *zoom, _BOOL init)
{
  ...
  if(ColorScheme.Call(&backhi, &p->dyncolback, GetBackColor_Makro(p->btx.colback)) == _TRUE)
  {
    if(ColorScheme.Call(&backlo, &p->dyncolfront, GetPenColor_Makro(p->btx.colback)) == _TRUE)
    {
      if(ColorScheme.Call(&framehi, &p->dyncoltl, GetBackColor_Makro(p->btx.colframe)) == _TRUE)
      {
        if(ColorScheme.Call(&framelo, &p->dyncolbr, GetPenColor_Makro(p->btx.colframe)) == _TRUE)
        {
          if(ColorScheme.Call(&txt, &p->dyncoltxt, GetPenColor_Makro(p->btx.coltxt)) == _TRUE)
          {
            pictext = p->btx;
...
_BOOL _SubPicture::Out_COL_POLY(_COL_POLY *p, _DOT *offset, _ZOOM *zoom, _BOOL init)
{
  ...
  if(ColorScheme.Call(&colhi, &p->dyncolback, GetBackColor_Makro(p->poly.color)) == _TRUE)
  {
    if(ColorScheme.Call(&collo, &p->dyncolfront, GetPenColor_Makro(p->poly.color)) == _TRUE)
    {
      if(ColorScheme.Call(&frame, &p->dyncolframe, GetBackColor_Makro(p->poly.colorframe)) == _TRUE)
      {
        polygon            = p->poly;
...
//    txt = get_text(&p->text.anything, &pic->TextPuffer, &p->text.memvarlabel, _TRUE);
  ...
  if(txt != NULL)
  {
    ...
    if(p->keycode[2] != _NOKEY)
    {
      ...
      {
        coltxt = tct;
        tct = MakeColor(INVISIBLE, LIGHTCYAN);
      }
     #endif
      _ROOM  tin = inlet;
...
_BOOL get_stationary_state(_BOOL *pst, _VARLABEL *pvl, _DWORD setval, _DWORD resetval)
{
  ...
  {
    if(VarList.GetValue(pvl, &value, _TRUE) == _TRUE)
    {
...
_BOOL _SubPicture::Out_BUTTON(_BUTTON *p, _DOT *offset, _ZOOM *zoom, _ATTRIB gatt)
{
  ...
  ColorScheme.Call(&colhi,   &p->dyncolback,  GetBackColor_Makro(p->col));
  ColorScheme.Call(&collo,   &p->dyncolfront, GetPenColor_Makro(p->col));
  ColorScheme.Call(&framehi, &p->dyncoltl,    GetBackColor_Makro(p->framecol));
  ColorScheme.Call(&framelo, &p->dyncolbr,    GetPenColor_Makro(p->framecol));
  ColorScheme.Call(&coltxt,  &p->dyncoltxt,   GetPenColor_Makro(p->coltxt));
...
_BOOL _SubPicture::Out_BUTTON(_BUTTON *p, _DOT *offset, _ZOOM *zoom, _ATTRIB gatt)
{
  _BOOL   retcode = _FALSE;
  _COLOR  frame, col;
  _COLOR  colhi, collo, framehi, framelo;
  _ATTRIB att;
  _ROOM   room;
  _DOT    o;
  _BOOL   onoff = p->state;
...
//  draw_BUTTON(p, &room, zoom, onoff, p->selected, att, col, frame);
  ...
  {
   #ifdef OK_SEEK_BUTTON
    if(button_image(&room, (state == _TRUE)? p->image_sel_press : p->image_sel_release) == _TRUE)
      outcursor = _FALSE;
   #else
    button_image(&room, (state == _TRUE)? p->image_sel_press : p->image_sel_release);
   #endif
  }
  else
  {
    // released, pressed image
    button_image(&room, (state == _TRUE)?  p->image_press : p->image_release);
  }

  // describing image
  button_image(&room, p->image_describe);
  // text

  // object
  if(p->anything.list == OBJLST)
  {
    if(position_touch_inlet(&o, zoom, &room, att, p->SubPicture, onoff) == _TRUE)
    {
      p->SubPicture->Draw(&o, zoom);
      retcode = _TRUE;
    }
  }
//else  // nedamoi drau denga
//  {
//    att = (att & (~T_BOUND))|(T_MIDBOUND|T_CENTERBOUND);
//    out_ANYTHING(&p->anything, &room, p->font, att, col, NULL);
//    retcode = _TRUE;
//  }

 #ifdef OK_SEEK_BUTTON
  if((p->selected == _TRUE)&&(outcursor == _TRUE))
  {
    col = SetUp.GetEditColorBack(_EDITACTIVE, p->col);
    draw_button_cursor(place, col, T_COPY|T_SOLID|T_INVRS);
  }
 #endif
...
//  _IVALUE value;
  ...
  //      if(p->act_momentary == false) // auskommentiert aufgrund wunsch appli: auch ein momentary button soll den state des servers anzeigen, egal ob gedrückt oder nicht
        ...
        {
          _CRC32 tmp;
          GetButtonText(&tmp, GetPICTURE(), p, p->state);
          if(tmp != p->reftxtcrc) // crc von buttontext vergleichen
          {
            redraw = _TRUE;
          }
        }
...
void _SubPicture::DoButton(_BUTTON *pt, _BOOL press_state, _Window *Window, _DOT *offset, _ZOOM *zoom, _BOOL lrm, _BOOL isrepeat)
{
  ...
  if(press_state == _TRUE)
  {
    ...
    if(pt->keycode[0] != _NOKEY)
    {
      ...
      if((pt->keycode[2] != _NOKEY) && (VarList.VarDummyGet(VARNO_ALTGRFLAG) != 0))
      {
        keco = pt->keycode[2];
...
_BOOL do_doubleclick_cursor(_BUTTON *pt)
{
  _BOOL retcode = _FALSE;
      ...
      if(pt->workstate == STATE_RUN)
      {
           ...
           (pe->dot.y >= room.xy1.y) && (pe->dot.y <= room.xy2.y))
        {
          if(access_button(pt, _FALSE, _SELECTOR_ONLY_HID) == _TRUE)
          {
            ...
            retcode = _TRUE;
            if(do_doubleclick_cursor(pt) == _TRUE)
              retcode = _FALSE;
/*
            if(double_click_BUTTON(pt) == _TRUE)
            {
              retcode = _TRUE;
              DoButton(pt, _TRUE, Window, &offset, zoom, pe->lrm);
              start_REPEAT(&pt->frepeat, pt->tip);
            }
*/
          }
          break;
...
//            if(double_click_BUTTON(pt) == _TRUE)
  ...
  else if(pe->ftype == _EVENT_TRIGGER)
  {
    ...
    {
      pt = &Picture.mask.membutton.ptr[i];
      if((pt->state == _TRUE) && (pt->workstate == STATE_RUN))
      {
        room = pt->room;
        Zoom_ROOM(&room, &offset, zoom);
...
void _SubPicture::DoAllNegEdge(_BOOL redraw)
{
  ...
  for(i=0; i<pm->no; i++)
  {
    ...
    {
      pt->state    = _FALSE;
      pt->oldstate = _TRUE;     // just to enable refresh !!
      if(redraw == _TRUE)
      {
...
_BOOL _SubPicture::Overload(_VARLABEL *ov)
{
  _BOOL retcode = _FALSE;

  if(valid_VARLABEL(ov) == _TRUE)
    retcode = overload_PICTUREMASK(&Picture.mask, ov);
