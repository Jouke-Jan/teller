...
_BOOL _File::Read_HEADER(_UDWORD *version, const _ASCII *idstring)
{
  ...
  {
    if((*(_UDWORD*)&header[4] == 64)&&(*(_UDWORD*)&header[12] == Project.GetCrc32()))
    {
      st = _TRUE;
      if(idstring != NULL)
...
/*
_UBYTE* Ram_XCHKBIT(_XCHKBIT *p, _UBYTE *ptr)
{
	p->seqno  = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
	p->invert = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);
  return(ptr);
}

_UBYTE* Ram_VARSINGLE(_VARSINGLE *p, _UBYTE *ptr)
{
  p->state = (_DEFVAL)(*(_UWORD*)ptr); ptr += sizeof(_UWORD);
  p->value = *(_DWORD*)ptr;  ptr += sizeof(_UDWORD);
  return(ptr);
}

_UBYTE* Ram_VARLABEL(_VARLABEL *p, _UBYTE *ptr)
{
  _UBYTE i;
  _VARSINGLE *pi;

  init_VARLABEL(p);
  p->no = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);
  for(i=0, pi=p->info; i<p->no; i++, pi++)
    ptr = Ram_VARSINGLE(pi, ptr);
  return(ptr);
}

_UBYTE* Ram_LINKVL(_LINKVL *p, _UBYTE *ptr)
{
  init_LINKVL(p);
  p->info = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);
  ptr = Ram_VARLABEL(&p->link, ptr);
  return(ptr);
}

_UBYTE* Ram_ANYTHING(_ANYTHING *p, _UBYTE *ptr)
{
  p->list = (_ANYLST)(*(_UWORD*)ptr); ptr += sizeof(_UWORD);
  p->no   = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  return(ptr);
}

_UBYTE* Ram_MEM_VARLABEL(_MEM_VARLABEL *p, _UBYTE *ptr)
{
  _UWORD    no, i;
  _UDWORD   size;
  _VARLABEL *pv;
  ...
  init_MEM_VARLABEL(p);
  no   = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  size = no * sizeof(_VARLABEL);
  if(Memory2((void**)&p->ptr, size) != 0)
  {
    p->no = no;
    for(i=0,pv=p->ptr; i<no; i++,pv++)
      ptr = Ram_VARLABEL(pv, ptr);
  }
  return(ptr);
}

_UBYTE* Ram_ANYPARA(_ANYPARA *p, _UBYTE *ptr)
{
  init_ANYPARA_Makro(p);
  ptr = Ram_ANYTHING(&p->anything, ptr);
 #if COMPILER_VERSION >= 23
  ptr = Ram_MEM_VARLABEL(&p->memvarlabel, ptr);
 #endif
  return(ptr);
}

_UBYTE* Ram_VARINFO(_VARINFO *p, _UBYTE *ptr)
{
 #if COMPILER_VERSION < 29
  _VARLABEL tmp;
  _UBYTE tmp1;
  ...
  p->vartype = (_VARTYPE)(*(_UWORD*)ptr); ptr += sizeof(_UWORD);
  p->classid = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  ptr = Ram_VARLABEL(&p->lo_limit, ptr);

 #if COMPILER_VERSION < 29
  init_VARLABEL(&tmp);
  ptr = Ram_VARLABEL(&tmp, ptr);
  free_VARLABEL(&tmp);
 #endif

  ptr = Ram_VARLABEL(&p->hi_limit, ptr);
  ...
  p->userconfig = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  p->accessno   = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  p->format     = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  p->station    = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);
  p->bitno      = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);

 #if COMPILER_VERSION < 29
  ptr += sizeof(_UBYTE);   // OberUnterGrenze umrechnen
 #endif

  p->funit           = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);
  p->statistic.group = *(_UDWORD*)ptr; ptr += sizeof(_UDWORD);
  p->POWPRUIC        = *(_UBYTE*)ptr; ptr += sizeof(_UBYTE);
  p->reftime         = *(_UWORD*)ptr; ptr += sizeof(_UWORD);

  p->POWPRUIC &= 0x46;

  ptr = Ram_ANYPARA(&p->text1, ptr);
  ptr = Ram_ANYPARA(&p->text2, ptr);
  ptr = Ram_ANYPARA(&p->text3, ptr);
  ptr = Ram_ANYPARA(&p->text4, ptr);

  p->funitscheme         = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
  p->statistic.parameter = *(_UDWORD*)ptr; ptr += sizeof(_UDWORD);
  p->stscheme            = *(_UWORD*)ptr; ptr += sizeof(_UWORD);

 #if COMPILER_VERSION >= 26
  p->uimenuindex = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
 #endif

 #if COMPILER_VERSION >= 29
  ptr = Ram_XCHKBIT(&p->xchkbit, ptr);
  no = *(_UWORD*)ptr; ptr += sizeof(_UWORD);
   ...
   if(no < UC_LINKVL)
   {
     for(i=0, plvl=p->linkvl; i<no; i++, plvl++)
       ptr = Ram_LINKVL(plvl, ptr);
   }
  #else
   _LINKVL tmplvl;
   init_LINKVL(&tmplvl);
