...
_BOOL DoEditorNeeds_COMBOBOX(_INPUT *pi)
{
  ...
  if((pcb != NULL) && (pi->io.info.tbo.list == MENULIST))
  {
    ...
    if(cindex != DEFSCOPE)
    {
      MenuList.DesignMenu(cindex, pi->io.room.xy2.x - pi->io.room.xy1.x + 1, pi->io.font, pcb->attrib, &pcb->info); //&pi->comboinfo);
//      pmen->text_color = mekeColor(pcombo->colback, pcombo->coltext);
